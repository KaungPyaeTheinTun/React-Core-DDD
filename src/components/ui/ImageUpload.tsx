...
function normalizeValue(value: (File | string)[] | undefined | null): (File | string)[] {
  if (!value) return [];
  return Array.isArray(value) ? value.filter(Boolean) : [value].filter(Boolean);
}

function getPreviewName(item: File | string, index: number): string {
...
export default function ImageUpload({
  ...
  useEffect(() => {
    ...
    const nextPreviews: Preview[] = selectedImages.map((item, index) => {
  ...
  const emitChange = (items: (File | string)[]) => {
    onChange(items);
  };
