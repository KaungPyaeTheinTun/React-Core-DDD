...
const readPermissionId = (item: any) => item?.id ?? item?.Id;

const readPermissionName = (item: any) => item?.name ?? item?.Name ?? "";
...
const extractPermissionIds = (response: any) => {
...
const isAdminRole = (roleId: string, name: string) =>
...
export default function RolesPermissionsPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        permRes = await permissionApi.getAll();
        if (cancelled) return;
        setAllPermissions(permRes.data?.items || []);
      } catch (err: unknown) {
        if (!cancelled) {
          setError((err as any)?.response?.data?.message || "Failed to load permissions");
          setLoading(false);
          return;
        }
      }
  ...
  const groupedPermissions: Record<string, any[]> = useMemo(() => {
  ...
  const toggleModule = useCallback(
    (moduleName: string, selectAll: boolean) => {
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      showNewCommentToast("System", (err as any)?.response?.data?.message || "Failed to save permissions.");
    } finally {
      setSaving(false);
    }
